...
void ofApp::update() {
	...
	for (int i = 0; i < 1; i++) {
		...
		auto deg_start = ofGetFrameNum() * 3;
		auto radius = 280;
	...
	for (int i = 0; i < this->location_list.size(); i++) {
		...
		if (this->life_list[i] > 10) {
			...
			this->setHexagonToMesh(this->face, this->frame, this->location_list[i], 12, height);
		}
		else if (this->life_list[i] > 0) {

			this->setHexagonToMesh(this->face, this->frame, this->location_list[i], ofMap(this->life_list[i], 0, 10, 0, 12), height);
		}
		else {
